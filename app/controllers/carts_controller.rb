class CartsController < ApplicationController
    ...
    def checkout
        ...
        if current_user.wallet.present? && current_user.wallet.balance >= total_revenue
            ActiveRecord::Base.transaction do
                ...
                if @order.save
                    ...
                    @cart.cart_items.each do |cart_item|
                        product_owner = cart_item.product.user
                        
                        if product_owner.wallet.present?
                            product_owner.wallet.update(balance: product_owner.wallet.balance + cart_item.price * cart_item.quantity )
                        else
                            product_owner.create_wallet(balance: cart_item.price * cart_item.quantity)
                        end
                    end
                    ...
                    flash[:danger] = "Your order could not be created. Please try again."
                    # redirect_to root_path
                end
            end
        else
        ...
        flash[:error] = "Some items in your cart are no longer available."
        redirect_to root_path
    end
